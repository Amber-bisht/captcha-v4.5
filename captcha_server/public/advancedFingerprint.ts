...
(function () {
    ...
    async function audioHash(): Promise<string> {
        try {
            ...
            osc.start(0);
            const fp = await new Promise<string>((res) => {
    ...
    async function fontsHash(): Promise<string> {
        ...
        for (const f of fonts) {
            if (measure(`'${f}', monospace`) !== baseWidth) detected.push(f);
        }
        return await hashStr(detected.join(','));
    }

    async function webrtcIPs(): Promise<string[]> {
        const ips: string[] = [];
        try {
            ...
            await new Promise<void>((res) => {
                ...
                pc.onicecandidate = (e) => {
                    if (!e.candidate) { clearTimeout(tm); res(); return; }
    ...
    function detectLies() {
        ...
        return {
            ...
            browser: (ua.includes('chrome') && !(window as any).chrome),
        };
    }

    async function generateAdvancedFingerprint(): Promise<AdvFingerprint> {
        const nav = navigator as any;
        const ch = await canvasHash();
        const wgl = webglData();
        const ah = await audioHash();
        const fh = await fontsHash();
        const plugins = Array.from(nav.plugins || []).map((p: any) => p.name).join(',');
    ...
    (window as any).generateAdvancedFingerprint = generateAdvancedFingerprint;
})();
