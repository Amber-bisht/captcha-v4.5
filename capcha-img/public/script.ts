...
function renderSecureGrid(images: SecureImage[]): void {
  if (!grid) return;
...
if (verifyBtn) {
  verifyBtn.addEventListener('click', async () => {
    if (!currentSessionId || !currentToken || !currentCsrfToken) return;
    ...
    try {
      ...
      const behaviorData = (window as any).behaviorTracker?.getData() || null;
